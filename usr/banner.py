...
{C}| .  . || |_/ / |_  {r}AUTHOR   {r}: ANGGA KURNIAWAN
{C}| |\/| || ___ \  _| {P}GITHUB   {r}: /ANGGAXD
{C}| |  | || |_/ / |   {C}FACEBOOK {r}: USERVIP.ANGGAXD
{C}\_|  |_/\____/\_|   {M}YOUTUBE  {r}: ANGGA KURNIAWAN
            """.format(C,r,G))
def menu():
    print(f"{G}1{r} Crack From Friendlist")
    print(f"{G}2{r} Crack From Likes On Post")
